...
def _resolve_dataset_default_path(default_path):
    custom_dataset_path = os.environ.get("LIBERO_DATASET_PATH")
    if not custom_dataset_path:
        return default_path

    resolved_path = os.path.abspath(os.path.expanduser(custom_dataset_path))
...
def _initialize_config_file():
    ...
    default_path_dict["datasets"] = _resolve_dataset_default_path(
        default_path_dict["datasets"]
    )
    os.makedirs(default_path_dict["datasets"], exist_ok=True)
